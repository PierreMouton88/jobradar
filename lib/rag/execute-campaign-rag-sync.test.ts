...
function createPlanItem(
  ...
  return {
    action,
    requiresEmbedding:
      options.requiresEmbedding ?? action !== "UP_TO_DATE",
    reasons:
      action === "UP_TO_DATE"
        ? []
        : action === "CREATE"
...
function createDependencies(
  ...
  return {
    ...
    saveEmbedding: vi.fn().mockImplementation(
      async (input) => ({
    ...
    updateWithoutEmbedding: vi.fn().mockImplementation(
      async (input) => ({
        sourceType: input.sourceType,
        sourceId: input.sourceId,
        title: input.title,
...
describe("executeCampaignRagSync", () => {
  it("reste en dry-run par défaut sans appel OpenAI ni écriture", async () => {
    ...
    expect(
      dependencies.generateEmbedding,
    ).not.toHaveBeenCalled();
    expect(
      dependencies.saveEmbedding,
    ).not.toHaveBeenCalled();
    expect(
      dependencies.updateWithoutEmbedding,
    ).not.toHaveBeenCalled();
  });
